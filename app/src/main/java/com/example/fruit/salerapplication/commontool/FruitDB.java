...
public class FruitDB {
    ...
    public static FruitDB instance = null;
    public static FruitDB getInstance(Context context){
        if(instance == null){
            instance = new FruitDB(context);
        }
        return instance;
    }
    ...
    public boolean deletePerference(int typeid) {
        ...
        try {
            ...
            Toast.makeText(context, "发生数据库错误：删除偏好失败", Toast.LENGTH_SHORT).show();
        } finally {
            if (db != null) {
                db.endTransaction();
                db.close();
    ...
    public boolean deleteStorage(int fsid) {
        ...
        try {
            ...
            Toast.makeText(context, "发生数据库错误：删除库存失败", Toast.LENGTH_SHORT).show();
        } finally {
            if (db != null) {
                db.endTransaction();
                db.close();
...
//        String sql = "Delete TABLE IF NOT EXISTS " + tableName + " (key text primary key, value text)";
//        db.execSQL(sql);
//    }
    ...
    public String querySettingValue(String key){
        for(SystemSettingBean bean: querySetting()){
